...
class Rs2010ForumRecentTopics_Widget extends WP_Widget {
    private $rs2010forum = null;

    public function __construct() {
        global $rs2010forum;
        $this->rs2010forum = $rs2010forum;
        $widget_ops = array('classname' => 'rs2010forumrecenttopics_widget', 'description' => __('Shows recent topics in Rs2010 Forum.', 'rs2010-forum'));
		parent::__construct('rs2010forumrecenttopics_widget', __('Rs2010 Forum: Recent Topics', 'rs2010-forum'), $widget_ops);
    }
        ...
        foreach ($elements as $element) {
            ...
            $output .= '<div class="widget-content">';
                ...
                if ($element->post_counter > 1) {
                    ...
                    $count_answers_i18n_text = ', '.sprintf(_n('%s Reply', '%s Replies', $answers, 'rs2010-forum'), $count_answers_i18n);
                }
                ...
                $output .= '<span class="post-date">'.sprintf(__('%s ago', 'rs2010-forum'), human_time_diff(strtotime($element->date), current_time('timestamp'))).$count_answers_i18n_text.'</span>';

                $custom_content = apply_filters('rs2010forum_widget_recent_topics_custom_content', '', $element->id);
    ...
    public function widget_output($args, $instance, $output) {
        // Generate title.
        $title = __('Recent Forum Topics', 'rs2010-forum');
    ...
    public function form($instance) {
        $title = isset($instance['title']) ? esc_attr($instance['title']) : __('Recent forum topics', 'rs2010-forum');
