...
class Rs2010ForumUserGroups {
    private static $rs2010forum = null;
    private static $taxonomyName = 'rs2010forum-usergroup';
    ...
    public function initialize() {
        ...
        if (self::$rs2010forum->permissions->isAdministrator(get_current_user_id())) {
    ...
    public static function initializeTaxonomy() {
        ...
        self::$taxonomyName = apply_filters('rs2010forum_filter_user_groups_taxonomy_name', self::$taxonomyName);
    }
    ...
    public static function insertUserGroupsOfUsers($user_id, $usergroups) {
        ...
        foreach ($usergroups as $usergroup) {
            if (!in_array($usergroup, $current_usergroups)) {
                do_action('rs2010forum_usergroup_'.$usergroup.'_add_user', $user_id, $usergroup);
            }
        }
        ...
        foreach ($current_usergroups as $usergroup) {
            if (!in_array($usergroup, $usergroups)) {
                do_action('rs2010forum_usergroup_'.$usergroup.'_remove_user', $user_id, $usergroup);
            }
        }
    }
    ...
    public static function getUserGroups($include = array(), $visible_groups_only = false) {
        ...
        $userGroups = array_filter($userGroups, array('Rs2010ForumUserGroups', 'getUserGroupsArrayFilter'));
    ...
    public static function canUserAccessForumCategory($userID, $forumCategoryID) {
        ...
        if (!self::$rs2010forum->permissions->isAdministrator($userID)) {
    ...
    public function manageUsersColumns($columns) {
        $columns['forum-user-groups'] = __('Forum Usergroups', 'rs2010-forum');
        return $columns;
  	}
    ...
    public static function renderUserGroupsInCategory($categoryID) {
        ...
        if (!empty($userGroupsOfForumCategory)) {
            echo ' &middot; '.__('Usergroups:', 'rs2010-forum').' ';
    ...
    public static function renderCategoryEditorFields() {
        ...
        if (!empty($userGroupCategories)) {
            echo '<tr id="usergroups-editor">';
                echo '<th><label>'.__('Usergroups:', 'rs2010-forum').'</label></th>';
                echo '<td>';
                    ...
                    echo '<span class="description">'.__('When usergroups are selected, only users of the selected usergroups will have access to the category.', 'rs2010-forum').'</span>';
                echo '</td>';
            echo '</tr>';
        }
    }
    ...
    public static function filterCategories($unfilteredCategories) {
        ...
        if (!self::$rs2010forum->permissions->isAdministrator('current') && !empty($filteredCategories) && !is_wp_error($filteredCategories)) {
    ...
    public static function showUserProfileFields($userID) {
        ...
        if (!empty($userGroupCategories)) {
            ...
            $output .= '<th><label>'.__('Usergroups', 'rs2010-forum').'</label></th>';
            $output .= '<td>';
    ...
    public static function render_usergroup_tag($usergroup_object, $font_weight = 'normal') {
        ...
        if (self::$rs2010forum->memberslist->functionality_enabled() && !is_admin()) {
            $link = self::$rs2010forum->rewrite->get_link('members', false, array('filter_type' => 'group', 'filter_name' => $usergroup_object->term_id));

            $output = '<a href="'.$link.'" class="rs-usergroup-tag usergroup-tag-'.$usergroup_object->term_id.'" style="color: '.$color.' !important; border-color: '.$color.' !important; font-weight: '.$font_weight.' !important;">';
            ...
            $output = '<span class="rs-usergroup-tag usergroup-tag-'.$usergroup_object->term_id.'" style="color: '.$color.' !important; border-color: '.$color.' !important; font-weight: '.$font_weight.' !important;">';
    ...
    public function views($views) {
        ...
        if ($usergroups) {
            $views['forum-user-group'] = '<b>'.__('Forum Usergroups:', 'rs2010-forum').'</b>&nbsp;';
    ...
    public function user_query($Query = '') {
        ...
        if (!self::$rs2010forum->prevent_query_modifications) {
    ...
    public function bulk_actions_users($bulk_actions) {
        ...
        if (!empty($userGroups)) {
            foreach ($userGroups as $usergroup) {
                $bulk_actions['forum_user_group_add_'.$usergroup->term_id] = __('Add to', 'rs2010-forum').' '.$usergroup->name;
            }

            foreach ($userGroups as $usergroup) {
                $bulk_actions['forum_user_group_remove_'.$usergroup->term_id] = __('Remove from', 'rs2010-forum').' '.$usergroup->name;
            }
        }
    ...
    public function bulk_actions_admin_notices() {
        if (!empty($_REQUEST['forum_user_groups_assigned'])) {
            printf('<div class="updated"><p>'.__('Usergroups assignments updated.', 'rs2010-forum').'</p></div>');
        }
    }
