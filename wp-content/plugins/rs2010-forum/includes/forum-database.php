...
class Rs2010ForumDatabase {
    ...
    public function buildSubsite($blog_id, $user_id, $domain, $path, $site_id, $meta) {
        ...
        if (is_plugin_active_for_network('rs2010-forum/rs2010-forum.php')) {
            switch_to_blog($blog_id);
            $this->setTables();
            $this->buildDatabase();
    ...
    public function buildDatabase() {
        ...
        $database_version_installed = get_option('rs2010forum_db_version');
        ...
        if ($database_version_installed != $this->db_version) {
            ...
            if ($first_time_installation) {
                ...
                if ($page_id && !is_wp_error($page_id)) {
                    $rs2010forum->load_options();
                    $rs2010forum->options['location'] = $page_id;
                    $rs2010forum->save_options($rs2010forum->options);
                }

                update_option('rs2010forum_db_version', 1);
            }
            ...
            if ($database_version_installed < 5) {
                ...
                update_option('rs2010forum_db_version', 5);
            }
            ...
            if ($database_version_installed < 6 && !$first_time_installation) {
                ...
                foreach ($forums as $forum) {
                    $slug = $rs2010forum->rewrite->create_unique_slug($forum->name, $this->tables->forums, 'forum');
                    $this->db->update($this->tables->forums, array('slug' => $slug), array('id' => $forum->id), array('%s'), array('%d'));
                }

                update_option('rs2010forum_db_version', 6);
            }
            ...
            if ($database_version_installed < 13) {
                ...
                Rs2010ForumUserGroups::initializeTaxonomy();

                // Create a new example category first.
                $defaultCategoryName = __('Custom Usergroups', 'rs2010-forum');
                $defaultCategory = Rs2010ForumUserGroups::insertUserGroupCategory($defaultCategoryName);

                // Ensure that no error happened.
                if (!is_wp_error($defaultCategory)) {
                    // Now get all existing elements.
                    $existingCategories = Rs2010ForumUserGroups::getUserGroupCategories();
                    ...
                    if (count($existingCategories) > 1) {
                        ...
                        foreach ($existingCategories as $category) {
                            ...
                            if ($category->term_id != $defaultCategory['term_id']) {
                                $color = Rs2010ForumUserGroups::getUserGroupColor($category->term_id);
                                Rs2010ForumUserGroups::updateUserGroup($category->term_id, $defaultCategory['term_id'], $category->name, $color);
                            }
                        }
                    } else {
                        // Add an example usergroup.
                        $defaultUserGroupName = __('Example Usergroup', 'rs2010-forum');
                        $defaultUserGroup = Rs2010ForumUserGroups::insertUserGroup($defaultCategory['term_id'], $defaultUserGroupName, '#256db3');
                    }
                }

                update_option('rs2010forum_db_version', 13);
            }
            ...
            if ($database_version_installed < 14 && !$first_time_installation) {
                ...
                $rs2010forum->load_options();
                $rs2010forum->appearance->load_options();

                // Build the intersect.
                $appearance_intersect = array_intersect_key($rs2010forum->options, $rs2010forum->appearance->options) + $rs2010forum->appearance->options;

                // Remove keys from old settings.
                $options_cleaned = array_diff_key($rs2010forum->options, $appearance_intersect);

                // Save all options.
                $rs2010forum->appearance->save_options($appearance_intersect);
                $rs2010forum->save_options($options_cleaned);

                update_option('rs2010forum_db_version', 14);
            }
            ...
            if ($database_version_installed < 19) {
                ...
                update_option('rs2010forum_db_version', 19);
            }

            // Create some default content.
            if ($database_version_installed < 20) {
                // Initialize taxonomy first.
                Rs2010ForumContent::initialize_taxonomy();

                // Get all categories first.
                $categories = $rs2010forum->content->get_categories(false);

                // Only continue when there are no categories yet.
                if (count($categories) == 0) {
                    // Add an example category.
                    $default_category_name = __('Example Category', 'rs2010-forum');

                    $new_category = wp_insert_term($default_category_name, 'rs2010forum-category');

                    if (!is_wp_error($new_category)) {
                        update_term_meta($new_category['term_id'], 'category_access', 'everyone');
                        update_term_meta($new_category['term_id'], 'order', 1);

                        $default_forum_name = __('First Forum', 'rs2010-forum');
                        $default_forum_description = __('My first forum.', 'rs2010-forum');

                        $rs2010forum->content->insert_forum($new_category['term_id'], $default_forum_name, $default_forum_description, 0, 'fas fa-comments', 1, 'normal');
                    }
                }

                update_option('rs2010forum_db_version', 20);
            }
            ...
            if ($database_version_installed < 23 && !$first_time_installation) {
                ...
                update_option('rs2010forum_db_version', 23);
            }
            ...
            if ($database_version_installed < 26 && !$first_time_installation) {
                ...
                if (!empty($get_moderators)) {
                    foreach ($get_moderators as $moderator) {
                        $rs2010forum->permissions->set_forum_role($moderator->ID, 'moderator');
                    }
                }
                ...
                if (!empty($get_banned)) {
                    foreach ($get_banned as $banned) {
                        $rs2010forum->permissions->set_forum_role($banned->ID, 'banned');
                    }
                }

                delete_metadata('user', 0, 'rs2010forum_banned', '', true);

                update_option('rs2010forum_db_version', 26);
            }
            ...
            if ($database_version_installed < 27 && !$first_time_installation) {
                ...
                update_option('rs2010forum_db_version', 27);
            }
            ...
            if ($database_version_installed < 33 && !$first_time_installation) {
                ...
                update_option('rs2010forum_db_version', 33);
            }
            ...
            if ($database_version_installed < 35 && !$first_time_installation) {
                ...
                update_option('rs2010forum_db_version', 35);
            }
            ...
            if ($database_version_installed < 36 && !$first_time_installation) {
                ...
                update_option('rs2010forum_db_version', 36);
            }
            ...
            if ($database_version_installed < 51 && !$first_time_installation) {
                ...
                update_option('rs2010forum_db_version', 51);
            }
            ...
            if ($database_version_installed < 53 && !$first_time_installation) {
                ...
                update_option('rs2010forum_db_version', 53);
            }
            ...
            if ($database_version_installed < 54 && !$first_time_installation) {
                ...
                update_option('rs2010forum_db_version', 54);
            }
            ...
            if ($database_version_installed < 58 && !$first_time_installation) {
                ...
                update_option('rs2010forum_db_version', 58);
            }
            ...
            if ($database_version_installed < 59 && !$first_time_installation) {
                ...
                update_option('rs2010forum_db_version', 59);
            }
            ...
            if ($database_version_installed < 61 && !$first_time_installation) {
                ...
                update_option('rs2010forum_db_version', 61);
            }
            ...
            if ($database_version_installed < 63 && !$first_time_installation) {
                ...
                update_option('rs2010forum_db_version', 63);
            }

            update_option('rs2010forum_db_version', $this->db_version);
        }
    }
}
