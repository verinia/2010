...
class Rs2010ForumPolls {
    private $rs2010forum = null;

    public function __construct($object) {
        $this->rs2010forum = $object;

        add_action('rs2010forum_editor_custom_content_bottom', array($this, 'editor_poll_form'), 10, 1);
        add_action('rs2010forum_after_add_topic_submit', array($this, 'editor_poll_process'), 10, 6);
        add_action('rs2010forum_after_edit_post_submit', array($this, 'editor_poll_process'), 10, 6);
        add_action('rs2010forum_prepare_topic', array($this, 'save_vote'));
        add_action('rs2010forum_after_delete_topic', array($this, 'delete_poll'), 10, 1);
    }

    public function editor_poll_form($editor_view) {
        // Cancel if poll-functionality is disabled.
        if (!$this->rs2010forum->options['enable_polls']) {
            return;
        }
        ...
        if ($this->rs2010forum->options['polls_permission'] == 'moderator' && !$this->rs2010forum->permissions->isModerator('current')) {
            return;
        }

        // Set IDs.
        $post_id = $this->rs2010forum->current_post;
        $topic_id = $this->rs2010forum->current_topic;
        ...
        if ($editor_view === 'editpost') {
            ...
            if (!$this->rs2010forum->is_first_post($post_id)) {
                return;
            }
        ...
        echo '<div class="editor-row">';
            ...
            echo '<div id="poll-form" style="display: none;">';
                echo '<div id="poll-question">';
                    echo '<input class="editor-subject-input" type="text" maxlength="255" name="poll-title" placeholder="'.__('Enter your question here', 'rs2010-forum').'" value="">';
                echo '</div>';
                ...
                echo '<div id="poll-options">';
                    ...
                    echo '<a class="poll-option-add">'.__('Add another answer', 'rs2010-forum').'</a>';
                    ...
                    echo '<div id="poll-warning">';
                        echo __('After creating a poll it will be no longer possible to add or remove answers. This ensures the integrity of existing votes. However, it will be still possible to make text-changes to existing answers and to poll-settings. If major changes for a poll are required, you must delete the existing poll and create a new one for this topic.', 'rs2010-forum');
                    echo '</div>';
                echo '</div>';

                echo '<div id="poll-settings">';
                    echo '<label class="checkbox-label">';
                        echo '<input type="checkbox" name="poll-multiple"><span>'.__('Allow multiple answers', 'rs2010-forum').'</span>';
                    echo '</label>';
                    echo '<span class="remove-poll">'.__('Remove Poll', 'rs2010-forum').'</span>';
                echo '</div>';
            echo '</div>';
        echo '</div>';
    }
    ...
    public function render_empty_poll_option_container() {
        echo '<div class="poll-option-container">';
            echo '<div class="poll-option-input">';
                echo '<input class="editor-subject-input" type="text" maxlength="255" name="poll-option[]" placeholder="'.__('An answer ...', 'rs2010-forum').'" value="">';
            echo '</div>';
    ...
    public function poll_form_edit($poll) {
        echo '<div class="editor-row">';
            ...
            echo '<div id="poll-form" style="display: block;">';
                echo '<div id="poll-question">';
                    echo '<input class="editor-subject-input" type="text" maxlength="255" name="poll-title" placeholder="'.__('Enter your question here', 'rs2010-forum').'" value="'.esc_html(stripslashes($poll->title)).'">';
                echo '</div>';
                ...
                echo '<div id="poll-options">';
                    foreach ($poll->options as $option) {
                        echo '<div class="poll-option-container">';
                            echo '<div class="poll-option-input">';
                                echo '<input class="editor-subject-input" type="text" maxlength="255" name="poll-option['.$option->id.']" placeholder="'.__('An answer ...', 'rs2010-forum').'" value="'.esc_html(stripslashes($option->title)).'">';
                            echo '</div>';
                        echo '</div>';
                    }

                    echo '<div id="poll-warning">';
                        echo __('To ensure the integrity of existing votes it is not possible to add or remove answers. However, it is still possible to make text-changes to existing answers and to poll-settings. If major changes for a poll are required, you must delete the existing poll and create a new one for this topic.', 'rs2010-forum');
                    echo '</div>';
                echo '</div>';

                echo '<div id="poll-settings">';
                    echo '<label class="checkbox-label">';
                        echo '<input type="checkbox" name="poll-multiple" '.checked($poll->multiple, 1, false).'><span>'.__('Allow multiple answers', 'rs2010-forum').'</span>';
                    echo '</label>';
                    echo '<span class="remove-poll">'.__('Remove Poll', 'rs2010-forum').'</span>';
                echo '</div>';
            echo '</div>';
    ...
    public function add_poll($topic_id, $title, $options, $multiple) {
        ...
        foreach ($options as $option) {
            $this->rs2010forum->db->insert(
                $this->rs2010forum->tables->polls_options,
    ...
    public function delete_poll($poll_id) {
        ...
        $this->rs2010forum->db->delete($this->rs2010forum->tables->polls, array('id' => $poll_id), array('%d'));
        $this->rs2010forum->db->delete($this->rs2010forum->tables->polls_options, array('poll_id' => $poll_id), array('%d'));
        $this->rs2010forum->db->delete($this->rs2010forum->tables->polls_votes, array('poll_id' => $poll_id), array('%d'));
    }
    ...
    public function save_vote() {
        ...
        $poll = $this->get_poll($this->rs2010forum->current_topic);
        ...
        foreach ($votes as $vote) {
            $this->rs2010forum->db->insert(
                $this->rs2010forum->tables->polls_votes,
    ...
    public function has_voted($user_id, $poll_id) {
        $has_voted = $this->rs2010forum->db->get_var("SELECT COUNT(*) FROM {$this->rs2010forum->tables->polls_votes} WHERE poll_id = {$poll_id} AND user_id = {$user_id};");
    ...
    public function render_poll($topic_id) {
        ...
        $can_see_results = $this->rs2010forum->options['polls_results_visible'];
            ...
            echo '<div id="poll-results">';
                        ...
                        echo '<div class="poll-result-name">';
                            ...
                            if ($can_see_results) {
                                echo '<small class="poll-result-numbers">';
                                    echo sprintf(_n('%s Vote', '%s Votes', $option->votes, 'rs2010-forum'), number_format_i18n($option->votes));
                                    echo '&nbsp;&middot;&nbsp;';
                ...
                if ($can_see_results) {
                    echo '<div class="poll-result-total">';
                        echo sprintf(_n('%s Participant', '%s Participants', $poll->total_participants, 'rs2010-forum'), number_format_i18n($poll->total_participants));
                    echo '</div>';
                }
                ...
                if ($can_vote) {
                    echo '<div class="actions">';
                        ...
                        echo '<input class="button button-normal" type="submit" value="'.__('Vote', 'rs2010-forum').'">';
                    echo '</div>';
                }
            echo '</div>';
        echo '</form>';
    ...
    public function has_poll($topic_id) {
        ...
        $poll = $this->rs2010forum->db->get_row("SELECT * FROM {$this->rs2010forum->tables->polls} WHERE id = {$topic_id};");
    ...
    public function get_poll($topic_id) {
        ...
        $poll->options = $this->rs2010forum->db->get_results("SELECT po.id, po.title, (SELECT COUNT(*) FROM {$this->rs2010forum->tables->polls_votes} AS pv WHERE pv.option_id = po.id) AS votes FROM {$this->rs2010forum->tables->polls_options} AS po WHERE po.poll_id = {$poll->id};", 'OBJECT_K');

        // Get total votes.
        $poll->total_votes = $this->rs2010forum->db->get_var("SELECT COUNT(*) FROM {$this->rs2010forum->tables->polls_votes} WHERE poll_id = {$poll->id};");

        // Get total participants.
        $participants = $this->rs2010forum->db->get_results("SELECT user_id, COUNT(*) FROM {$this->rs2010forum->tables->polls_votes} WHERE poll_id = {$poll->id} GROUP BY user_id;");
