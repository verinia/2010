...
class Rs2010ForumMentioning {
    ...
    public function __construct($object) {
        ...
        add_action('rs2010forum_enqueue_css_js', array($this, 'enqueue_css_js'));
    ...
    public function enqueue_css_js() {
        ...
        if (!$this->rs2010forum->options['enable_mentioning_suggestions']) {
            return;
        }

        $themeurl = $this->rs2010forum->appearance->get_current_theme_url();

        wp_enqueue_script('jquery-caret', $this->rs2010forum->plugin_url.'libs/jquery.caret.js', array('jquery'), $this->rs2010forum->version, true);
        wp_enqueue_script('jquery-atwho', $this->rs2010forum->plugin_url.'libs/jquery.atwho.js', array('jquery', 'jquery-caret'), $this->rs2010forum->version, true);
        wp_enqueue_script('rs2010forum-js-mentioning', $this->rs2010forum->plugin_url.'js/script-mentioning.js', array('jquery', 'jquery-atwho', 'wp-api'), $this->rs2010forum->version, true);

        wp_enqueue_style('rs2010forum-css-mentioning', $themeurl.'/style-mentioning.css', array(), $this->rs2010forum->version);
    }
    ...
    public function add_mentioning_to_editor($settings) {
        ...
        $settings['init_instance_callback'] = 'window.rs2010.suggestions_initialize';
    ...
    public function mentioning_callback($data) {
        ...
        $user_query = new Rs2010ForumUserQuery($user_query);
		$response['data'] = array();
    ...
    private function create_link($match) {
        ...
        if ($user) {
            $link = $this->rs2010forum->renderUsername($user, '@'.$match[1]).$match[2];
        }
    ...
    public function user_wants_notification($user_id) {
        $mention_user = get_user_meta($user_id, 'rs2010forum_mention_notify', true);
        ...
        if (!empty($matches)) {
            foreach ($matches as $match) {
                ...
                if ($user && $this->user_wants_notification($user->ID)) {
                    $this->rs2010forum->notifications->add_to_mailing_list($user->user_email);
                }
            }

            if (!empty($this->rs2010forum->notifications->mailing_list)) {
                // Set receivers-list.
                $receivers = $this->rs2010forum->notifications->mailing_list;
                ...
                $notification_subject = $this->rs2010forum->options['mail_template_mentioned_subject'];
                $notification_message = $this->rs2010forum->options['mail_template_mentioned_message'];
                $notification_message = apply_filters('rs2010forum_filter_notify_mentioned_user_message', $notification_message, $replacements);
