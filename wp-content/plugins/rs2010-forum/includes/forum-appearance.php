...
class Rs2010ForumAppearance {
	private $theme_path = 'themes-rs2010forum';
	...
	public function load_options() {
		// Load options.
		$this->options = array_merge($this->options_default, get_option('rs2010forum_appearance', array()));
	...
	public function save_options($options) {
		update_option('rs2010forum_appearance', $options);
	...
	private function find_themes() {
		...
		$this->themes[$this->default_theme] = array(
			'name'	=> __('Default Theme', 'rs2010-forum'),
			'url'	=> $this->rs2010forum->plugin_url.$this->skin_path
		);
	...
	public function set_header() {
		...
		if ($this->rs2010forum->executePlugin) {
			echo '<!-- Rs2010 Forum - SEO: BEGIN -->'.PHP_EOL;

			$link = ($this->rs2010forum->current_page > 0) ? $this->rs2010forum->get_link('current') : esc_url(remove_query_arg('part', $this->rs2010forum->get_link('current', false, false, '', false)));
			$title = ($this->rs2010forum->getMetaTitle()) ? $this->rs2010forum->getMetaTitle() : get_the_title();

			// By default use the page title as description.
			$description = $title;

			// In forum overview use the forum description if available.
			if ($this->rs2010forum->current_view === 'overview' && !empty($this->rs2010forum->options['forum_description'])) {
				$description = $this->rs2010forum->options['forum_description'];
			} else if ($this->rs2010forum->current_description && $this->rs2010forum->error === false) {
				$description = $this->rs2010forum->current_description;
			}
			...
			if (in_array($this->rs2010forum->current_view, $blocked_views_for_searchengines)) {
			...
			if ($this->rs2010forum->options['hide_profiles_from_guests'] && in_array($this->rs2010forum->current_view, $profile_views)) {
				$prevent_indexing = true;
			}
			...
			if ($this->rs2010forum->current_view === 'topic' && $this->rs2010forum->current_element) {
				$first_post = $this->rs2010forum->content->get_first_post($this->rs2010forum->current_element);

				if ($first_post) {
					$image_url = $this->rs2010forum->extract_image_url($first_post->text);
			...
			do_action('rs2010forum_wp_head');

			echo '<!-- Rs2010 Forum - SEO: END -->'.PHP_EOL;
		}
	}
	...
	public function add_css() {
		if ($this->rs2010forum->executePlugin) {
			...
			$custom_css_path = $this->rs2010forum->plugin_path.'skin/custom.css';
			...
			if ($this->is_default_theme() && $this->options != $this->options_default) {
				...
				if (!empty($custom_css)) {
					...
					if (file_exists($custom_css_path) && filesize($custom_css_path)) {
						...
						$current_css = $this->rs2010forum->read_file($custom_css_path);
						...
						if (!$current_css) {
							...
							if ($hash_new != $hash_current) {
								$file_check = $this->rs2010forum->create_file($custom_css_path, $custom_css);
					...
					if ($inline) {
						...
						// Load CSS inline.
						wp_add_inline_style('rs-style', $custom_css);
					} else {
						// Load CSS as file.
						wp_enqueue_style('rs-custom-color', $this->get_current_theme_url().'/custom.css', array(), $this->rs2010forum->version);
					}
				}
			} else {
				// Otherwise try to delete the custom CSS file.
				$this->rs2010forum->delete_file($custom_css_path);
			}
		}
	}
	...
	public function add_editor_css($mce_css) {
		...
		$mce_css .= $this->get_current_theme_url().'/editor.css?ver='.$this->rs2010forum->version;
	...
	public function generate_custom_css() {
		...
		if ($this->options['custom_color'] != $this->options_default['custom_color'] && preg_match('/#([a-fA-F0-9]{3}){1,2}\b/', $this->options['custom_color'])) {
			$custom_css .= '#rs-wrapper #forum-profile .display-name,'.PHP_EOL;
			$custom_css .= '#rs-wrapper input[type="checkbox"]:checked:before {'.PHP_EOL;
				$custom_css .= 'color: '.$this->options['custom_color'].' !important;'.PHP_EOL;
			$custom_css .= '}'.PHP_EOL;

			$custom_css .= '#rs-wrapper .button-normal,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .title-element,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #forum-header,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #profile-header .background-avatar,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #profile-navigation,'.PHP_EOL;
			$custom_css .= '#rs-wrapper input[type="radio"]:checked:before {'.PHP_EOL;
				$custom_css .= 'background-color: '.$this->options['custom_color'].' !important;'.PHP_EOL;
			$custom_css .= '}'.PHP_EOL;

			$custom_css .= '#rs-wrapper input[type="radio"]:focus,'.PHP_EOL;
			$custom_css .= '#rs-wrapper input[type="checkbox"]:focus,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #profile-header {'.PHP_EOL;
		...
		if ($this->options['custom_accent_color'] != $this->options_default['custom_accent_color'] && preg_match('/#([a-fA-F0-9]{3}){1,2}\b/', $this->options['custom_accent_color'])) {
			$custom_css .= '#rs-wrapper .button-normal,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .title-element,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #forum-navigation a,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #forum-navigation-mobile a {'.PHP_EOL;
				$custom_css .= 'border-color: '.$this->options['custom_accent_color'].' !important;'.PHP_EOL;
			$custom_css .= '}'.PHP_EOL;

			$custom_css .= '#rs-wrapper #profile-navigation a.active {'.PHP_EOL;
		...
		if ($this->options['custom_text_color'] != $this->options_default['custom_text_color'] && preg_match('/#([a-fA-F0-9]{3}){1,2}\b/', $this->options['custom_text_color'])) {
			$custom_css .= '#rs-wrapper,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #forum-breadcrumbs a:hover,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .main-title {'.PHP_EOL;
		...
		if ($this->options['custom_text_color_light'] != $this->options_default['custom_text_color_light'] && preg_match('/#([a-fA-F0-9]{3}){1,2}\b/', $this->options['custom_text_color_light'])) {
			$custom_css .= '#rs-wrapper .main-title:before,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .editor-row-uploads .upload-hints,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .forum-stats,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .topic-stats,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .action-panel-description,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #forum-breadcrumbs,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #forum-breadcrumbs a,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .forum-post-date,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .post-footer,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .post-footer a,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .signature,'.PHP_EOL;
			$custom_css .= '#rs-wrapper span.mention-nice-name,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .post-reactions .reaction,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #poll-results .poll-result-numbers,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #poll-results .poll-result-total,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #poll-warning,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .topic-icon:before,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .report-link,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .report-content:before,'.PHP_EOL;
			$custom_css .= '#rs-wrapper input::placeholder,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .activity-time,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .unread-time {'.PHP_EOL;
		...
		if ($this->options['custom_link_color'] != $this->options_default['custom_link_color'] && preg_match('/#([a-fA-F0-9]{3}){1,2}\b/', $this->options['custom_link_color'])) {
			$custom_css .= '#rs-wrapper a,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .forum-post-menu a,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .post-author .topic-author,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #bottom-navigation {'.PHP_EOL;
		...
		if ($this->options['custom_background_color'] != $this->options_default['custom_background_color'] && preg_match('/#([a-fA-F0-9]{3}){1,2}\b/', $this->options['custom_background_color'])) {
			$custom_css .= '#rs-wrapper .content-container,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .report-element,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #statistics,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .post-wrapper,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #profile-header .background-contrast,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #poll-results .poll-result-bar,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #memberslist-filter {'.PHP_EOL;
		...
		if ($this->options['custom_background_color_alt'] != $this->options_default['custom_background_color_alt'] && preg_match('/#([a-fA-F0-9]{3}){1,2}\b/', $this->options['custom_background_color_alt'])) {
			$custom_css .= '#rs-wrapper .content-element:nth-child(even),'.PHP_EOL;
			$custom_css .= '#rs-wrapper .topic-sticky,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .topic-sticky .topic-poster,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .post-element,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .editor-element,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #statistics-online-users,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #profile-layer,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .spoiler .spoiler-head,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .report-content,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #poll-panel,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #profile-content {'.PHP_EOL;
		...
		if ($this->options['custom_border_color'] != $this->options_default['custom_border_color'] && preg_match('/#([a-fA-F0-9]{3}){1,2}\b/', $this->options['custom_border_color'])) {
			$custom_css .= '#rs-wrapper input,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .forum-poster,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .topic-poster,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .member-last-seen,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .editor-element,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .content-container,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .forum-post-header,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #statistics-body,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .statistics-element,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #statistics-online-users,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .editor-row,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .editor-row-subject,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .signature,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .post-element,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .post-wrapper,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .forum-subforums,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .uploaded-file img,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .action-panel-option,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .topic-sticky .topic-poster,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #profile-layer,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #profile-layer .pages-and-menu:first-of-type,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #profile-content,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #profile-content .profile-row,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .history-element,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #memberslist-filter,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .content-element,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .ad-forum,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .ad-topic,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .spoiler,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .spoiler .spoiler-body,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .report-element,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .report-source,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .report-content,'.PHP_EOL;
			$custom_css .= '#rs-wrapper .report-actions,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #profile-content .profile-section-header,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #poll-options,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #poll-panel,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #poll-panel #poll-headline,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #poll-results .poll-result-bar,'.PHP_EOL;
			$custom_css .= '#rs-wrapper #usergroups-filter {'.PHP_EOL;
		...
		if ($this->options['custom_read_indicator_color'] != $this->options_default['custom_read_indicator_color'] && preg_match('/#([a-fA-F0-9]{3}){1,2}\b/', $this->options['custom_read_indicator_color'])) {
			$custom_css .= '#rs-wrapper .read {'.PHP_EOL;
				$custom_css .= 'color: '.$this->options['custom_read_indicator_color'].' !important;'.PHP_EOL;
			$custom_css .= '}'.PHP_EOL;

			$custom_css .= '#rs-wrapper #read-unread .read {'.PHP_EOL;
		...
		if ($this->options['custom_unread_indicator_color'] != $this->options_default['custom_unread_indicator_color'] && preg_match('/#([a-fA-F0-9]{3}){1,2}\b/', $this->options['custom_unread_indicator_color'])) {
			$custom_css .= '#rs-wrapper .unread {'.PHP_EOL;
				$custom_css .= 'color: '.$this->options['custom_unread_indicator_color'].' !important;'.PHP_EOL;
			$custom_css .= '}'.PHP_EOL;

			$custom_css .= '#rs-wrapper #read-unread .unread {'.PHP_EOL;
