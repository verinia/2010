...
class Rs2010ForumPermissions {
    ...
    public function __construct($object) {
        ...
        add_action('rs2010forum_prepare_profile', array($this, 'change_ban_status'));
    ...
    public function getForumRole($userID) {
        if ($this->isAdministrator($userID)) {
            return __('Administrator', 'rs2010-forum');
        } else if ($this->isModerator($userID)) {
            return __('Moderator', 'rs2010-forum');
        } else if ($this->isBanned($userID)) {
            return __('Banned', 'rs2010-forum');
        } else {
            return __('User', 'rs2010-forum');
        }
    }

    public function get_forum_role($user_id) {
        $role = get_user_meta($user_id, 'rs2010forum_role', true);
    ...
    public function set_forum_role($user_id, $role) {
        ...
        if (!$this->isSiteAdministrator($user_id)) {
            switch ($role) {
                case 'normal':
                    delete_user_meta($user_id, 'rs2010forum_role');
                break;
                case 'moderator':
                    update_user_meta($user_id, 'rs2010forum_role', 'moderator');
                break;
                case 'administrator':
                    update_user_meta($user_id, 'rs2010forum_role', 'administrator');
                break;
                case 'banned':
                    update_user_meta($user_id, 'rs2010forum_role', 'banned');
                break;
            }
        }
    }
    ...
    public function can_edit_post($user_id, $post_id, $author_id = false, $post_date = false) {
        ...
        if (!$this->rs2010forum->options['enable_edit_post']) {
            return false;
        }

        // Disallow when user is not the author of a post.
        $author_id = ($author_id) ? $author_id : $this->rs2010forum->get_post_author($post_id);
        ...
        $time_limitation = $this->rs2010forum->options['time_limit_edit_posts'];
        ...
        $date_creation = ($post_date) ? $post_date : $this->rs2010forum->get_post_date($post_id);
        $date_creation = strtotime($date_creation);
        $date_now = strtotime($this->rs2010forum->current_time());
    ...
    public function can_delete_topic($user_id, $topic_id, $author_id = false, $post_date = false) {
        ...
        if (!$this->rs2010forum->options['enable_delete_topic']) {
            return false;
        }

        // Get information about first post.
        $first_post = $this->rs2010forum->content->get_first_post($topic_id);
        ...
        // Allow when there is no time limitation.
        $time_limitation = $this->rs2010forum->options['time_limit_delete_topics'];
    ...
    public function can_delete_post($user_id, $post_id, $author_id = false, $post_date = false) {
        ...
        if (!$this->rs2010forum->options['enable_delete_post']) {
            return false;
        }
        ...
        // Allow when there is no time limitation.
        $time_limitation = $this->rs2010forum->options['time_limit_delete_posts'];
    ...
    public function can_open_topic($user_id, $topic_id) {
        ...
        if (!$this->rs2010forum->options['enable_open_topic']) {
            return false;
        }
    ...
    public function can_close_topic($user_id, $topic_id) {
        ...
        if (!$this->rs2010forum->options['enable_close_topic']) {
            return false;
        }
    ...
    public function can_create_post($user_id) {
        ...
        if (!$this->rs2010forum->is_topic_closed($this->rs2010forum->current_topic)) {
    ...
    public function can_use_signature($user_id) {
        ...
        if ($this->rs2010forum->options['signatures_permission'] == 'loggedin') {
            return true;
        }
    ...
    public function get_users_by_role($role) {
        ...
        $this->rs2010forum->prevent_query_modifications = true;

        switch ($role) {
            case 'all':
                $query = new Rs2010ForumUserQuery(array('fields' => array('ID', 'display_name')));
                $data = $query->results;
            break;
            case 'role':
                $query = new Rs2010ForumUserQuery(array('fields' => array('ID', 'display_name'), 'meta_key' => 'rs2010forum_role'));
                $data = $query->results;
            break;
            case 'siteadmin':
                $query = new Rs2010ForumUserQuery(array('fields' => array('ID', 'display_name'), 'role' => 'administrator'));
                $data = $query->results;
            break;
            case 'normal':
            ...
            break;
            case 'moderator':
                $query = new Rs2010ForumUserQuery(array('fields' => array('ID', 'display_name'), 'meta_key' => 'rs2010forum_role', 'meta_value' => 'moderator'));
            ...
            break;
            case 'administrator':
                $query = new Rs2010ForumUserQuery(array('fields' => array('ID', 'display_name'), 'meta_key' => 'rs2010forum_role', 'meta_value' => 'administrator'));
            ...
            break;
            case 'banned':
                $query = new Rs2010ForumUserQuery(array('fields' => array('ID', 'display_name'), 'meta_key' => 'rs2010forum_role', 'meta_value' => 'banned'));
        ...
        $this->rs2010forum->prevent_query_modifications = false;
    ...
    public function manage_users_columns($columns) {
        $columns['forum-user-role'] = __('Forum Role', 'rs2010-forum');
        return $columns;
  	}
    ...
    public function permission_views($views) {
        $views['forum-user-role'] = '<b>'.__('Forum Roles:', 'rs2010-forum').'</b>&nbsp;';

        // Normal users.
        $users = $this->get_users_by_role('normal');
        $cssClass = (!empty($_GET['forum-user-role']) && $_GET['forum-user-role'] == 'normal') ? 'class="current"' : '';
        $views['forum-user-role'] .= '<a '.$cssClass.' href="'.admin_url('users.php?forum-user-role=normal').'">'.__('Users', 'rs2010-forum').'</a> ('.count($users).')';
        ...
        $views['forum-user-role'] .= '<a '.$cssClass.' href="'.admin_url('users.php?forum-user-role=moderator').'">'.__('Moderators', 'rs2010-forum').'</a> ('.count($users).')';
        ...
        $views['forum-user-role'] .= '&nbsp;|&nbsp;';
        $views['forum-user-role'] .= '<a '.$cssClass.' href="'.admin_url('users.php?forum-user-role=administrator').'">'.__('Administrators', 'rs2010-forum').'</a> ('.count($users).')';

        // Banned.
        $users = $this->get_users_by_role('banned');
        $cssClass = (!empty($_GET['forum-user-role']) && $_GET['forum-user-role'] == 'banned') ? 'class="current"' : '';
        $views['forum-user-role'] .= '&nbsp;|&nbsp;';
        $views['forum-user-role'] .= '<a '.$cssClass.' href="'.admin_url('users.php?forum-user-role=banned').'">'.__('Banned', 'rs2010-forum').'</a> ('.count($users).')';
    ...
    public function permission_user_query($Query = '') {
        ...
        if (!$this->rs2010forum->prevent_query_modifications) {
    ...
    public function bulk_actions_users($bulk_actions) {
        $bulk_actions['forum_role_assign_normal'] = __('Assign forum role:', 'rs2010-forum').' '.__('User', 'rs2010-forum');
        $bulk_actions['forum_role_assign_moderator'] = __('Assign forum role:', 'rs2010-forum').' '.__('Moderator', 'rs2010-forum');
        $bulk_actions['forum_role_assign_administrator'] = __('Assign forum role:', 'rs2010-forum').' '.__('Administrator', 'rs2010-forum');
        $bulk_actions['forum_role_assign_banned'] = __('Assign forum role:', 'rs2010-forum').' '.__('Banned', 'rs2010-forum');
    ...
    public function bulk_actions_admin_notices() {
        if (!empty($_REQUEST['forum_role_assigned'])) {
            printf('<div class="updated"><p>'.__('Forum role assigned.', 'rs2010-forum').'</p></div>');
        }
    }
}
