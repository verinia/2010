...
class Rs2010ForumRewrite {
    ...
    function __construct($object) {
        ...
        if ($this->rs2010forum->options['enable_seo_urls'] && get_option('permalink_structure')) {
    ...
    private function build_view_mapping() {
        $this->view_mapping['activity']      = $this->rs2010forum->options['view_name_activity'];
        $this->view_mapping['subscriptions'] = $this->rs2010forum->options['view_name_subscriptions'];
        $this->view_mapping['search']        = $this->rs2010forum->options['view_name_search'];
        $this->view_mapping['forum']         = $this->rs2010forum->options['view_name_forum'];
        $this->view_mapping['topic']         = $this->rs2010forum->options['view_name_topic'];
        $this->view_mapping['addtopic']      = $this->rs2010forum->options['view_name_addtopic'];
        $this->view_mapping['movetopic']     = $this->rs2010forum->options['view_name_movetopic'];
        $this->view_mapping['addpost']       = $this->rs2010forum->options['view_name_addpost'];
        $this->view_mapping['editpost']      = $this->rs2010forum->options['view_name_editpost'];
        $this->view_mapping['markallread']   = $this->rs2010forum->options['view_name_markallread'];
        $this->view_mapping['members']       = $this->rs2010forum->options['view_name_members'];
        $this->view_mapping['profile']       = $this->rs2010forum->options['view_name_profile'];
        $this->view_mapping['history']       = $this->rs2010forum->options['view_name_history'];
        $this->view_mapping['unread']        = $this->rs2010forum->options['view_name_unread'];
        $this->view_mapping['unapproved']    = $this->rs2010forum->options['view_name_unapproved'];
        $this->view_mapping['reports']       = $this->rs2010forum->options['view_name_reports'];
    }
    ...
    private function ensure_rewrite_rules() {
        if ($this->use_permalinks) {
            ...
            $pattern = $this->generate_rewrite_rule_pattern($this->rs2010forum->options['location']);
    ...
    function add_rewrite_rules_array($rules) {
        ...
        $page_ids = $this->rs2010forum->db->get_col('SELECT ID FROM '.$this->rs2010forum->db->prefix.'posts WHERE post_type = "page" AND (post_content LIKE "%[forum%" OR post_content LIKE "%[Forum%");');
    ...
    function disable_front_page_redirect($requested_url, $do_redirect) {
        ...
        if (isset($post)) {
            if (get_option('show_on_front') === 'page') {
                ...
                if ($front_page_id == $post->ID && $front_page_id == $this->rs2010forum->options['location']) {
                    $requested_url = false;
                }
            }
        }
    ...
    function parse_url() {
        ...
        if (!empty($_GET['view'])) {
            ...
            if ($key == false) {
                $this->rs2010forum->current_view = esc_html($_GET['view']);
            } else {
                $this->rs2010forum->current_view = $key;
            }
        }

        // Set the current element id.
        if (!empty($_GET['id'])) {
            $this->rs2010forum->current_element = absint($_GET['id']);
        }

        // Set the current page.
        if (isset($_GET['part']) && absint($_GET['part']) > 0) {
            $this->rs2010forum->current_page = (absint($_GET['part']) - 1);
        }

        // Fallback for old view-name.
        if ($this->rs2010forum->current_view == 'thread') {
        ...
        if ($this->use_permalinks) {
            ...
            if (!empty($parsed_url[0])) {
                ...
                if ($key == false) {
                    $this->rs2010forum->current_view = esc_html($parsed_url[0]);
                } else {
                    $this->rs2010forum->current_view = $key;
                }
            }
            ...
            if (!empty($parsed_url[1])) {
                ...
                if (is_numeric($parsed_url[1])) {
                    $this->rs2010forum->current_element = absint($parsed_url[1]);

                    // But this does not hold for usernames because they can be numeric as well.
                    if ($this->rs2010forum->current_view == 'profile' || $this->rs2010forum->current_view == 'history') {
                        // Only make the conversion when slugs are used for profile-URLs.
                        if ($this->rs2010forum->options['seo_url_mode_profile'] === 'slug') {
                            $this->rs2010forum->current_element = $this->convert_slug_to_id($parsed_url[1], $this->rs2010forum->current_view);
                        }
                    }
                } else {
                    $this->rs2010forum->current_element = $this->convert_slug_to_id($parsed_url[1], $this->rs2010forum->current_view);
    ...
    function maybe_301_redirect() {
        ...
        // search engines.
        if ($this->rs2010forum->current_view) {
            $redirect_link = $this->get_link($this->rs2010forum->current_view, $this->rs2010forum->current_element);

            if ($this->rs2010forum->current_page) {
                $redirect_link = add_query_arg(array('part' => ($this->rs2010forum->current_page + 1)), $redirect_link);
            }
        ...
        if (isset($this->links[$type])) {
            ...
            if ($element_id) {
                if ($this->use_permalinks) {
                    ...
                    $trailing_slash = apply_filters('rs2010forum_seo_trailing_slash', '/');
    ...
    function get_post_link($post_id, $topic_id = false, $post_page = false, $additional_parameters = array()) {
        ...
        if (!$topic_id) {
            $topic_id = $this->rs2010forum->db->get_var($this->rs2010forum->db->prepare("SELECT parent_id FROM {$this->rs2010forum->tables->posts} WHERE id = %d;", $post_id));
        }
        ...
        if (!$post_page) {
            ...
            if (empty($this->cache_get_post_link_ids[$topic_id])) {
                $this->cache_get_post_link_ids[$topic_id] = $this->rs2010forum->db->get_col("SELECT id FROM {$this->rs2010forum->tables->posts} WHERE parent_id = ".$topic_id." ORDER BY id ASC;");
            }
            ...
            $post_page = ceil($post_position / $this->rs2010forum->options['posts_per_page']);
        }
    ...
    function set_links() {
        ...
        // otherwise the generated links would not be correct when the forum is located on a static front page.
        $this->links['home'] = untrailingslashit(_get_page_link($this->rs2010forum->options['location']));
    ...
    function create_unique_slug($name, $location, $type) {
        ...
        if (empty($this->slug_cache[$type])) {
            $this->slug_cache[$type] = $this->rs2010forum->db->get_col("SELECT slug FROM ".$location." WHERE slug <> '';");
        }
    ...
    function convert_slug_to_id($slug, $type) {
        ...
        if (empty($this->convert_slug_to_id_cache[$type.'-'.$slug])) {
            ...
            switch ($type) {
                case 'topic':
                    $result = $this->rs2010forum->db->get_var('SELECT id FROM '.$this->rs2010forum->tables->topics.' WHERE slug = "'.$slug.'";');
                    ...
                    break;
                case 'forum':
                    $result = $this->rs2010forum->db->get_var('SELECT id FROM '.$this->rs2010forum->tables->forums.' WHERE slug = "'.$slug.'";');
        ...
        if (empty($this->convert_id_to_slug_cache[$type.'-'.$id])) {
            ...
            switch ($type) {
                case 'topic':
                    ...
                    if ($this->rs2010forum->options['seo_url_mode_content'] === 'id') {
                        break;
                    }

                    $result = $this->rs2010forum->db->get_var('SELECT slug FROM '.$this->rs2010forum->tables->topics.' WHERE id = '.$id.';');
                ...
                case 'forum':
                    ...
                    $result = $this->rs2010forum->db->get_var('SELECT slug FROM '.$this->rs2010forum->tables->forums.' WHERE id = '.$id.';');
                ...
                case 'profile':
                    ...
                    if ($this->rs2010forum->options['seo_url_mode_profile'] === 'id') {
                        break;
                    }
