...
class Rs2010ForumApproval {
    private $rs2010forum = null;

    public function __construct($object) {
        $this->rs2010forum = $object;

        add_action('rs2010forum_breadcrumbs_unapproved', array($this, 'add_breadcrumbs'));
        add_action('rs2010forum_prepare_overview', array($this, 'notice_for_topic_creator'));
        add_action('rs2010forum_prepare_overview', array($this, 'notice_for_moderators'));
    }

    public function add_breadcrumbs() {
        $element_link = $this->rs2010forum->get_link('unapproved');
        $element_title = __('Unapproved Topics', 'rs2010-forum');
    ...
    public function is_topic_approved($topic_id) {
        ...
        if (!isset($this->is_topic_approved_cache[$topic_id])) {
            $approved = $this->rs2010forum->db->get_var("SELECT approved FROM {$this->rs2010forum->tables->topics} WHERE id = {$topic_id};");
    ...
    public function approve_topic($topic_id) {
        if ($this->rs2010forum->permissions->isModerator('current')) {
            ...
            $this->rs2010forum->db->update($this->rs2010forum->tables->topics, array('approved' => 1), array('id' => $topic_id), array('%d'), array('%d'));
            ...
            $receivers = $this->rs2010forum->mentioning->mention_users($first_post->id);

            // Send notifications about new topic.
            $this->rs2010forum->notifications->notify_about_new_topic($topic_id, $receivers);

            do_action('rs2010forum_after_topic_approve', $topic_id);
        }
    }
    ...
    public function get_unapproved_topics($forum_id = 'all') {
        if ($forum_id === 'all') {
            ...
            if (!empty($ids_categories)) {
                ...
                return $this->rs2010forum->db->get_results("SELECT t.id, t.name, f.name AS forum_name FROM {$this->rs2010forum->tables->topics} AS t LEFT JOIN {$this->rs2010forum->tables->forums} AS f ON (t.parent_id = f.id) WHERE f.parent_id IN ({$ids_categories}) AND t.approved = 0 ORDER BY t.id ASC;");
            }
        } else {
            return $this->rs2010forum->db->get_results("SELECT * FROM {$this->rs2010forum->tables->topics} WHERE approved = 0 AND parent_id = {$forum_id} ORDER BY id DESC;");
        }
    ...
    public function notify_about_new_unapproved_topic($topic_id) {
        ...
        $notification_subject = __('New unapproved topic', 'rs2010-forum');
        ...
        $notification_message = __('Hello ###USERNAME###,<br><br>You received this message because there is a new unapproved forum-topic.<br><br>Topic:<br>###TITLE###<br><br>Author:<br>###AUTHOR###<br><br>Text:<br>###CONTENT###<br><br>Link:<br>###LINK###', 'rs2010-forum');
    ...
    public function forum_requires_approval($forum_id, $user_id) {
        ...
        if ($this->rs2010forum->permissions->isModerator($user_id)) {
            return false;
        }

        // Check if the forum requires approval for new topics.
        $status = $this->rs2010forum->db->get_var("SELECT forum_status FROM {$this->rs2010forum->tables->forums} WHERE id = {$forum_id};");
        ...
        if ($status == 'approval') {
            ...
            if ($this->rs2010forum->options['approval_for'] == 'normal') {
                return true;
            }
        }
    ...
    public function notice_for_topic_creator() {
        if (!empty($_GET['new_unapproved_topic'])) {
            $notice = __('Thank you for your topic. Your topic will be visible as soon as it gets approved.', 'rs2010-forum');
            $link = false;
            $icon = 'fas fa-eye';
    ...
    public function notice_for_moderators() {
        ...
        if (!empty($unapproved_topics)) {
            $notice = __('There are unapproved topics.', 'rs2010-forum');
            $link = $this->rs2010forum->rewrite->get_link('unapproved');
            $icon = 'fas fa-eye';
            $this->rs2010forum->add_notice($notice, $link, $icon);
        }
    }
    ...
    public function show_unapproved_topics() {
        ...
        // Render pagination.
        $pagination_rendering = $this->rs2010forum->pagination->renderPagination('unapproved', false, $unapproved_topics_counter);
        ...
        if ($unapproved_topics_counter > 0) {
            ...
            foreach ($data_sliced as $topic) {
                ...
                $first_post = $this->rs2010forum->content->get_first_post($topic->id);
                ...
                echo '<div class="content-element unapproved-topic topic-normal">';
                    ...
                    echo '<div class="topic-name">';
                        echo '<a href="'.$this->rs2010forum->rewrite->get_link('topic', $topic->id).'" title="'.$topic_title.'">'.$topic_title.'</a>';
                        echo '<small>';

                        // Author
                        echo __('By', 'rs2010-forum').'&nbsp;'.$this->rs2010forum->getUsername($first_post->author_id);

                        // Creation time
                        echo '&nbsp;&middot;&nbsp;';
                        echo sprintf(__('%s ago', 'rs2010-forum'), human_time_diff(strtotime($first_post->date), current_time('timestamp')));
            ...
            $this->rs2010forum->render_notice(__('There are no unapproved topics.', 'rs2010-forum'));
        }
