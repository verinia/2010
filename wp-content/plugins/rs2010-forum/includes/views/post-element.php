...
echo '<div class="post-element '.$this->permissions->getForumRole($post->author_id).'-post '.$first_post_class.'" id="postid-'.$post->id.'">';
    echo '<div class="post-author '.$user_online_class.'">';
        // Show avatar if activated.
        if ($this->options['enable_avatars']) {
            $avatar_size = apply_filters('rs2010forum_filter_avatar_size', 120);
            echo get_avatar($post->author_id, $avatar_size, '', '', array('force_display' => true));
        }

        echo '<div class="post-author-block-name">';
            // Show username.
            $username = apply_filters('rs2010forum_filter_post_username', $this->getUsername($post->author_id), $post->author_id);
        ...
        if ($user_data != false) {
            echo '<div class="post-author-block-meta">';
                ...
                if ($this->options['show_author_posts_counter']) {
                    ...
                    echo '<small class="post-counter">'.sprintf(_n('%s Post', '%s Posts', $post->author_posts, 'rs2010-forum'), $author_posts_i18n).'</small>';
                }
                ...
                if ($this->current_view != 'post' && $this->options['highlight_authors'] && ($counter > 1 || $this->current_page > 0) && $topicStarter != 0 && $topicStarter == $post->author_id) {
                    echo '<small class="topic-author">'.__('Topic Author', 'rs2010-forum').'</small>';
                }

                // Show marker for banned user.
                if ($this->permissions->isBanned($post->author_id)) {
                    echo '<small class="banned">'.__('Banned', 'rs2010-forum').'</small>';
                }
            echo '</div>';

            // Show groups of user.
            $usergroups = Rs2010ForumUserGroups::getUserGroupsOfUser($post->author_id, 'all', true);
        ...
        do_action('rs2010forum_after_post_author', $post->author_id, $post->author_posts);
    echo '</div>';
    ...
    echo '<div class="post-wrapper">';
        ...
        echo '<div class="post-message">';
            ...
            echo '<div id="post-quote-container-'.$post->id.'" style="display: none;"><blockquote><div class="quotetitle">'.__('Quote from', 'rs2010-forum').' '.$this->getUsername($post->author_id).' '.sprintf(__('on %s', 'rs2010-forum'), $this->format_date($post->date)).'</div>'.wpautop($post_content).'</blockquote><br></div>';
            ...
            $post_content = apply_filters('rs2010forum_filter_post_content', $post_content, $post->id);

            echo $post_content;
            echo $this->uploads->show_uploaded_files($post->id, $post->uploads);

            do_action('rs2010forum_after_post_message', $post->author_id, $post->id);
        echo '</div>';
            ...
            echo '<div class="post-footer">';
                ...
                echo '<div class="post-meta">';
                    if ($this->options['show_edit_date'] && (strtotime($post->date_edit) > strtotime($post->date))) {
                        ...
                        if ($post->author_edit) {
                            echo sprintf(__('Last edited on %s by %s', 'rs2010-forum'), $this->format_date($post->date_edit), $this->getUsername($post->author_edit));
                        } else {
                            echo sprintf(__('Last edited on %s', 'rs2010-forum'), $this->format_date($post->date_edit));
                        }
...
do_action('rs2010forum_after_post');
?>
